...
df_fp_hpp = df_fp1.loc[(df_fp1['tech']=='Fierza')]
...
graph_title = 'FP scenario - Change in electricity generation in Fierza under different opr. rules'

fig4b = px.bar(df_fp_hpp2, x='year', y='value', text='value', color='scenario',barmode='group', 
...
fig4b.for_each_annotation(lambda a: a.update(text=a.text.split("=")[-1]))
fig4b.update_traces(texttemplate='%{text:.4s}', textposition='inside') #to format the text on each bar

fig4b.update_yaxes(range=[1650, 1850]) #setting the y-axis scale to ensure enough space for the text on each bar